...
categories: ctf re
---
First were given an address to connect to. Upon connecting with `nc` we see a bunch of (AT&T :s) x86_64 assembly, and we are told to delete lines to get under a certain threshold without changing the return value. Easy enough! I backtrack from the last write to eax and trace back until I see its final immediate value being written to the stack. Submit those lines, easy flag.

Congrats! Your flag is PCTF{iN3ffic1eent__A5m_K1ll5}

Now on to the next one. This one, as expected, is more of the same. Connecting a few times I see that the assembly changes. @uafio an I try to download all of them and start working on them manually. In 1000 iteration we only found 100 unique assembly listings... That's reasonable. While working through, though, the sense that this isn't it won't leave me. We'll have to automate this.

Now I know in theory how this is done. I've read on [Static single assignment form](https://en.wikipedia.org/wiki/Static_single_assignment_form) before. The idea is to re-version every variable every time its assigned to, and track which version of which variable was used in determining its current value.
