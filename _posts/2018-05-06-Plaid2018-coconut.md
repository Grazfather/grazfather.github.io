...
Now we obviously need the directives (lines 1 and 2), plus the stack setup (lines 3 and 4) plus the return (line 19). Other than these lines, and the lines mentioned above, we can delete everything else! If we submit these line numbers, we shoud get a flag.
